...
## Milestone 6: private academic materials

Each subject now has a Materials section with drag-and-drop, a file picker, transfer progress, a private-storage saving state, metadata listing, and confirmed deletion. PDF, PNG, JPG/JPEG, and WEBP are accepted up to 20 MB (20 × 1024 × 1024 bytes). Both client and server validate size, extension, and MIME; the server also checks the file signature. Signatures are a type check, not malware scanning. Files are not rendered or analyzed.

Storage uses the private `academic-materials` bucket and generated `user-id/subject-id/random-uuid.extension` paths. The bucket enforces a 20 MB limit and four MIME types. The two new migrations configure Storage and qualify the subject lookup in the insert policy; both are applied remotely. Existing migrations and public-table policies are unchanged. Storage INSERT checks `auth.uid()` against the path and subject owner, SELECT/DELETE permit only the owner's prefix, and no UPDATE policy permits overwrites. This follows [Supabase Storage RLS](https://supabase.com/docs/guides/storage/security/access-control).

`src/app/(dashboard)/subjects/[id]/materials/route.ts` verifies the session, origin, subject ownership, UUIDs, and bounded multipart request bytes. It uses only the existing authenticated Supabase client. `next.config.ts` permits 22 MB proxy buffering for the file plus multipart overhead; deployment proxies must also support requests of this size. Hosts with smaller request limits need a separate direct-upload design before deployment.

Metadata is reserved before upload so uncertain Storage failures remain discoverable. Failed uploads attempt blob cleanup, then metadata cleanup. Failed cleanup retains the record with an error instructing deletion/retry. Material deletion removes the blob first and keeps metadata if cleanup fails; retries tolerate an already-missing blob. These are compensating operations, not an atomic Storage/database transaction. Subject deletion requires removing materials first. Concurrent deletion outside the application or interrupted requests can still require owner/admin cleanup; no background cleanup worker is introduced.

Tests: `npm test` covers PDF/image upload, invalid MIME/extensions/signatures, oversized uploads, ownership, CSRF, listing, and deletion/cleanup failures. `supabase/tests/material_storage.sql` checks the actual bucket settings and Storage policies transactionally with rolled-back metadata fixtures (no file blobs); it passed on the linked project. Existing database security tests still cover materials ownership and foreign keys. The HTTP application tests use a local Storage fixture, not live file uploads. No new environment variables, service-role keys, AI integration, or content extraction are added.
