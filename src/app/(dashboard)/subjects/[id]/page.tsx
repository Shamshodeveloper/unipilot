...
export default async function SubjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const materials = await listMaterials(subject.id);
  return <main id="main-content" className="mx-auto w-full max-w-6xl flex-1 px-5 py-12 sm:px-8">
    ...
    <MaterialManager subjectId={subject.id} materials={materials} />
    <div className="mt-8 grid gap-6 sm:grid-cols-2">{["Topics", "Exams", "Learning Progress"].map((title) => <DashboardSection key={title} title={title} description={`Your subject’s ${title.toLowerCase()}.`}><EmptyState title={`${title} is coming later.`} detail="This section is not available yet." /></DashboardSection>)}</div>
